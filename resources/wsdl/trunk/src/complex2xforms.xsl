  ...
  <xsl:choose>
    ...
    <xsl:when test="exists($complex/*)">
      ...
      <xsl:variable name="enumeration">
        <xsl:apply-templates select="$complex/xtee:substitution[tm:extract-label(@*:to) = $formname and tm:extract-path(@*:to) = $currentref and tm:extract-condition(@*:to) = '[@enumeration]']" mode="enumeration">
...
<xsl:template match="xtee:substitution" mode="submission-actions-submit-done-url">
  ...
  <xsl:param name="operation" tunnel="yes"/>
  <xsl:param name="tnsprefix" tunnel="yes"/>
  <xforms:setvalue ref="instance('{$formname}.output')/SOAP-ENV:Body/{$tnsprefix}:{$operation}Response/keha/url" value="concat(instance('{$formname}.output')/SOAP-ENV:Body/{$tnsprefix}:{$operation}Response/keha/url, '&amp;{substring-before(substring-after(@*:to, '#xpointer(/url/'), ')')}=', instance('{$formname}.input')/SOAP-ENV:Body/{$tnsprefix}:{$operation}/url/{substring-before(substring-after(@*:to, '#xpointer(/url/'), ')')})" events:event="xforms-submit-done"/>
</xsl:template>
...
<xsl:template match="xtee:arc[@*:actuate = 'onRequest' and @*:to != '']" mode="link">
  ...
  <xforms:group>
    ...
    <xsl:if test="upper-case(@*:title) = '_DATA' and $node/xsd:annotation/xsd:appinfo/xtee:title">
...
<xsl:template match="xtee:arc[@*:actuate = 'onLoad' and @*:to != '']" mode="link">
  ...
  <xsl:choose>
    <xsl:when test="exists(@*:arcrole)">
      ...
      <xforms:toggle case="{@*:to}.request" events:event="xforms-select" if="boolean(instance('{$formname}.output')/SOAP-ENV:Body/{$tnsprefix}:{$operation}Response{concat(tm:extract-path(@*:arcrole), tm:extract-condition(@*:arcrole))})"/>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template match="xtee:substitution" mode="substitutions">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$from-form = $formname and not(starts-with($from-path, '/'))">
      <xforms:setvalue ref="instance('{$to-form}.input')/SOAP-ENV:Body/{$to-producer}:{$to-operation}{$to-path}" value="context()/{$from-path}"/>
    </xsl:when>
...
<xsl:template match="xtee:substitution" mode="enumeration">
  ...
  <xforms:select1 ref="{$ref}">
    ...
    <xforms:itemset nodeset="instance('{$from-form}.output')/SOAP-ENV:Body/{$from-producer}:{$from-operation}Response{$from-path}">
      <xforms:label ref="*"/>
      <xforms:value ref="*"/>
    </xforms:itemset>
  </xforms:select1>
