...
<xsl:template match="wsdl:binding/wsdl:operation">
  ...
  <xsl:if test="($operation = '' or $operation = @name) and @name != 'listMethods' and @name != 'loadClassificators' and @name != 'getCharge'">
    ...
    <xsl:variable name="formname" select="$operation"/>
...
<xsl:template match="/" mode="bind">
  ...
  <xsl:choose>
    ...
    <xsl:when test="ends-with($what, 'Type') and $localname = 'ehak' and $namespace = 'http://x-tee.riik.ee/xsd/xtee.xsd' and $formtype = 'input'">
      <!-- Use first attribute that is not empty as EHAK value. -->
      <xsl:attribute name="calculate">if(@asula != '', @asula, if(@vald != '', @vald, @maakond))</xsl:attribute>
...
<xsl:template match="xsd:element | xsd:attribute" mode="required">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$formtype = 'input'">
      <xsl:if test="$requirecontent = 'true' and @name != 'keha' and (exists(@minOccurs) = false() or @minOccurs &gt; 0 or @use='required')">true()</xsl:if>
      <!--xsl:apply-imports /-->
    </xsl:when>
    <xsl:otherwise>
      <xsl:apply-imports />
      ...
      <xsl:choose>
          ...
          <xsl:choose>
            ...
            <xsl:when test="$localname = 'vald' and $namespace = 'http://x-tee.riik.ee/xsd/xtee.xsd'">
              <xforms:select1 ref="{$ref}">
                ...
                <xforms:itemset nodeset="instance('vallad.classifier')/maakond{if ($dependref) then concat('[@kood=', $parentref, '/', $dependref, ']') else ''}/vald">
                  <xforms:label ref="@nimi"/>
                  <xforms:value ref="@kood"/>
                </xforms:itemset>
              </xforms:select1>
            ...
            <xsl:when test="$localname = 'asula' and $namespace = 'http://x-tee.riik.ee/xsd/xtee.xsd'">
              <xforms:select1 ref="{$ref}">
                ...
                <xsl:apply-templates select="$node" mode="label"/>
                <xsl:variable name="dependref" select="$node/xsd:annotation/xsd:appinfo/xtee:ref"/>
                <xforms:itemset nodeset="instance('asulad.classifier')/maakond/vald{if ($dependref) then concat('[@kood=', $parentref, '/', $dependref, ']') else ''}/asula">
                  <xforms:label ref="@nimi"/>
                  <xforms:value ref="@kood"/>
                </xforms:itemset>
              </xforms:select1>
            ...
            <xsl:when test="$localname = 'ehak' and $namespace = 'http://x-tee.riik.ee/xsd/xtee.xsd'">
              <xforms:group ref="{$ref}">
                ...
                <xforms:select1 ref="@vald">
                  ...
                  <xforms:itemset nodeset="instance('ehak.classifier')/maakond[@kood={$parentref}/{$ref}/@maakond]/vald">
                    <xforms:label ref="@nimi"/>
                    <xforms:value ref="@kood"/>
                  </xforms:itemset>
                ...
                <xforms:select1 ref="@asula">
                  ...
                  <xforms:itemset nodeset="instance('ehak.classifier')/maakond[@kood={$parentref}/{$ref}/@maakond]/vald[@kood={$parentref}/{$ref}/@vald]/asula">
                    <xforms:label ref="@nimi"/>
                    <xforms:value ref="@kood"/>
                  </xforms:itemset>
                </xforms:select1>
        ...
        <xsl:when test="$formtype = 'output'">
          <xsl:choose>
            ...
            <xsl:when test="$localname = 'maakond' and $namespace = 'http://x-tee.riik.ee/xsd/xtee.xsd'">
              <xforms:output value="instance('maakonnad.classifier')/maakond[@kood = current()/{$ref}]/@nimi">
                <!-- Generate labels and hints. -->
                <xsl:apply-templates select="$node" mode="label"/>
              </xforms:output>
            </xsl:when>
            <xsl:when test="$localname = 'vald' and $namespace = 'http://x-tee.riik.ee/xsd/xtee.xsd'">
              ...
              <xforms:output value="instance('vallad.classifier')/maakond{if ($dependref) then concat('[@kood=', $parentref, '/', $dependref, ']') else ''}/vald[@kood = current()/{$ref}]/@nimi">
                <!-- Generate labels and hints. -->
                <xsl:apply-templates select="$node" mode="label"/>
              </xforms:output>
            </xsl:when>
            <xsl:when test="$localname = 'asula' and $namespace = 'http://x-tee.riik.ee/xsd/xtee.xsd'">
              ...
              <xforms:output value="instance('asulad.classifier')/maakond/vald{if ($dependref) then concat('[@kood=', $parentref, '/', $dependref, ']') else ''}/asula[@kood = current()/{$ref}]/@nimi">
                <!-- Generate labels and hints. -->
                <xsl:apply-templates select="$node" mode="label"/>
              </xforms:output>
            </xsl:when>
...
<xsl:template match="xsd:element" mode="form">
  ...
  <xsl:choose>
    ...
    <xsl:when test="normalize-space(xsd:annotation/xsd:appinfo/xtee:visibility) = '0'">
    </xsl:when>
    <xsl:otherwise>
      <xsl:apply-imports/>
    </xsl:otherwise>
...
<xsl:template match="wsdl:message" mode="form">
  ...
  <xsl:choose>
    <xsl:when test="$formtype = 'input'">
      <xsl:variable name="element" select="wsdl:part[@name = 'keha']/@element"/>
      <xsl:variable name="type" select="wsdl:part[@name = 'keha']/@type"/>
      <xsl:variable name="qname" select="resolve-QName($type, .)"/>
      <xsl:variable name="localname" select="local-name-from-QName($qname)"/>
      <xsl:variable name="namespace" select="namespace-uri-from-QName($qname)"/>
      <xsl:choose>
        <!-- Query will be submitted immediately, if keha type is missing or string. -->
...
<xsl:template match="wsdl:operation" mode="submission-actions-submit-done">
  ...
  <xsl:choose>
    <xsl:when test="starts-with($operation, 'legacy')">
      ...
      <xforms:load ref="instance('{$formname}.output')/SOAP-ENV:Body/{$tnsprefix}:{$operation}Response/keha/url" show="new" events:event="xforms-submit-done"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:apply-imports/>
    </xsl:otherwise>
...
<xsl:template match="wsdl:output" mode="form-fault">
  ...
  <xsl:param name="operation" tunnel="yes"/>
  <xsl:param name="tnsprefix" tunnel="yes"/>
  <xforms:group ref="instance('{$formname}.output')/SOAP-ENV:Body/{$tnsprefix}:{$operation}Response[not(keha/*)]" class="info">
